...
module.exports.run = async (client, message, args, reply) => {
  ...
  if (!bot) return reply("<:wrong:631497559424237600> Please specify a bot to reject.");
  if (!reason) return reply("<:wrong:631497559424237600> Please specify a reason for the rejection.");

  const bot1 = await bots.findOne({ id: bot });
  await bots.findOneAndDelete({ id: bot });
  
  if (!bot1) return reply("<:wrong:631497559424237600> The specified bot was not found in the queue.")
  ...
  client.channels.get("631739811647979540").send(`<@${bot}> by ${allOwners.join(" ")} has been rejected by ${message.author}.`);
