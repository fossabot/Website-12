...
module.exports.run = async (client, message, args, reply) => {
  ...
  if (!userProfile || userProfile.admin !== true)
    return reply(
      `<:wrong:631497559424237600> You aren't allowed to perform this action.`
    );
  var bot = message.mentions.users.first() || { id: args[0] };
  if (bot) bot = bot.id;
  if (!bot)
    return reply(
      "<:wrong:631497559424237600> Please specify a bot to feature."
    );
  if (!args[1]) return reply(`Please provide an argument \`add/del\``);
  if (args[1] !== "add" && args[1] !== "del")
    return reply(
      `<:wrong:631497559424237600> Please provide a valid argument (\`add/del\`)`
    );
  await bots.findOne({ id: bot }, async (err, res) => {
    if (err) console.log(err);
    if (!res)
      return reply(
        `<:wrong:631497559424237600> The specified bot wasn't found.`
      );
    ...
    if (args[1] == "del")
      reply(`${res.name} was removed from the featured bots.`);
  });
};
